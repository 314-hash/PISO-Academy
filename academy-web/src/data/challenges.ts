...
4. Emits the standard Transfer(address(0), to, amount) event on minting.
`,
...
2. Calling transferFrom MUST revert with "PISOCert: Soulbound non-transferable".
...
`,
    testCases: [
      { id: 'test-1', name: 'Token Minting', description: 'Owner can mint a new soulbound certificate', points: 30 },
    ...
    hints: [
      'Sa soulbound tokens, ang locked function ay laging nagbabalik ng true.',
  ...
  },
  {
    ...
    instructionsMarkdown: `
### Layunin ng Hamon
Implement an on-chain faucet with cooldown security:
1. Allow any user to request 1 PISO (DRIP_AMOUNT).
2. Store the timestamp of the last drip per address in \`lastDripTime\`.
3. Revert if the user requests again within 24 hours (COOLDOWN).
`,
    startingCode: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

contract BayanihanFaucetChallenge {
    uint256 public constant DRIP_AMOUNT = 1 ether;
...
`,
  ...
  },
  {
    ...
    instructionsMarkdown: `
### Layunin ng Hamon
Connect a consumer smart contract to the PISO AI Oracle:
1. Query \`getLatestInference(promptHash)\` from the oracle contract.
2. Assert that \`confidenceBps >= minConfidence\`.
3. Return the verified AI inference string.
`,
    startingCode: `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
`,
    testCases: [
      { id: 'test-1', name: 'Queries Oracle Contract', description: 'Calls IPISOAIOracle at configured address', points: 50 },
      { id: 'test-2', name: 'Validates Confidence Threshold', description: 'Reverts if confidence is lower than minConfidence', points: 50 }
    ],
    hints: [
      'Gamitin ang IPISOAIOracle(oracleAddress).getLatestInference(promptHash);'
    ]
  }
];
